	...
	user	*u;
{
	...
	switch (fork()) {
	case -1:
		log_it("PCRON",getpid(),"error","can't fork");
		break;
	case 0:
		/* child process */
		pidfile_close(pfh);
	...
	user	*u;
{
	...
	switch (vfork()) {
	case -1:
		log_it("PCRON",getpid(),"error","can't vfork");
		exit(ERROR_EXIT);
		/*NOTREACHED*/
	case 0:
...
# endif /*DEBUGGING*/
// BSE: Use execlp instead of execle so that the child process will inherit
// its environment from pcron.
//			execle(shell, shell, "-c", e->cmd, (char *)0, e->envp);
			execlp(shell, shell, "-c", e->cmd, (char *)0);
...
/* BSE: Don't mail to Unix mailbox */
#ifdef MAIL_TO_UNIX_BOX
			} else {
			...
			if (mailto) {
				...
				fprintf(mail, "From: %s (pcron)\n", usernm);
				fprintf(mail, "To: %s\n", mailto);
				fprintf(mail, "Subject: Pcron <%s@%s> %s\n",
